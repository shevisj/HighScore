...
- **`src/scores/`** — the core domain. `ScoreController` (mounted at `/api/scores`) handles CRUD; `ScoreService` does all DB work via MongoDB aggregation pipelines; `Score` is the Mongoose model + schema; `ScoreMiddleware` runs before the write routes (`POST`/`PUT`) to strip profanity from `name`; `WriteAuthMiddleware` runs before it on all write routes (`POST`/`PUT`/`DELETE`) to optionally gate writes behind `HIGHSCORE_WRITE_PASSWORD` (raw password, or an HMAC token bound to the payload when `HIGHSCORE_WRITE_TOKEN=true`). It must precede `ScoreMiddleware` so token verification sees the raw, unfiltered `name`.
...
- `ScoreService.addScore` normally inserts. When `HIGHSCORE_DEDUPE_SCORES=true` it instead upserts (`findOneAndUpdate` + `$setOnInsert`) keyed on `name` + `value` + `category` (session ignored), making `POST` idempotent for retries. With the same flag, `ScoreService.$onInit` provisions a unique compound index (`name_value_category_unique`) on those fields to enforce dedup at the DB level and close the concurrent-race window; the E11000 path is handled. Index creation is skipped (logged) if the collection still holds duplicates.
